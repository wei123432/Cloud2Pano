...
int main()
{
	//加载可视模型包围盒
	Mesh visibleMesh;
    if (!loadOBJNoSRS("D:\\experience\\Web\\Visualmodel\\test_visualmodel\\test\\visible_cam_50.obj", visibleMesh))
    {
...
#pragma omp critical
    ...
    for (int j = 0; j < bvhs.size();j++)
    {
        ...
        if (bvhs[j].nodes.empty()) 
            continue;

		//如果瓦块根节点与可视包围盒没有交集，则跳过
        const AABB& tileBox = bvhs[j].nodes[0].box;
        ...
        queryBVH_AABB(bvhs[j], 0, visibleBox, candidateTris);
    ...
    if (!saveOBJ("D:\\experience\\Web\\data_middle\\VisualModel\\tile_filter_7.obj", fullMesh))
    {
        std::cerr << "Failed to save merged cropped mesh." << std::endl;
        return -1;
    }

    std::cout << "Merged cropped mesh saved. "
